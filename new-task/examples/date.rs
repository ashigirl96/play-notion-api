...
fn main() {
    ...
    // let x = S("2022-03-14")
    let x = "2022-03-15";
    // let x = x.parse::<DateTimeUtc>().unwrap();
    ...
    eprintln!("match date_only = {:#?}", match date_only {
        Ok(x) => { x }
        Err(_) => { panic!("fack")}
    });

    eprintln!("match date_only = {:#?}", match hoge {
        Ok(x) => { x }
        Err(_) => { now }
    });
    ...
    let x = date_only;
    eprintln!("x = {:#?}", x);
    // eprintln!("date_only = {:#?}", date_only.unwrap().day());
    // eprintln!("date_only = {:#?}", date_only.unwrap().month());
    // eprintln!("date_only = {:#?}", date_only.unwrap().year());

    // match date_only {
    //     ParseResult(x) => eprintln!("x = {:#?}", x),
    //     _ => eprintln!("hoge = {:#?}", "hoge"),
    // }

    // eprintln!("x = {}", x.0.day());
    //
    // let x = "now";
    // // let x = x.parse::<DateTimeUtc>();
    // if let Result(x) = x.parse() {
    //
    // }
    //
    // eprintln!("x = {:#?}", x);
